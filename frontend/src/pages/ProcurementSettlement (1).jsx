...
export default function ProcurementSettlement() {
  ...
  const [advances, setAdvances] = useState([]); // AvailableAdvanceOut[]
  ...
  const previewMath = useMemo(() => {
    ...
    const paidNow = +(net - advanceTotal).toFixed(2);
    return { gross, deduction, net, advanceTotal, paidNow };
  }, [preview, deductionPercent, advanceTotal]);
  ...
  const loadClientContext = async (cid) => {
    if (!cid) { setSettlements([]); setLastSettlement(null); setAdvances([]); setSelectedAdvanceIds([]); return; }
    const [hist, last, adv] = await Promise.allSettled([
      api.get(`/procurement/settlements?client_id=${cid}`),
      api.get(`/procurement/clients/${cid}/last-settlement`),
      api.get(`/procurement/clients/${cid}/advances`),
    ]);
    if (hist.status === "fulfilled") setSettlements(hist.value.data || []);
    else { setSettlements([]); toast.error(formatApiError(hist.reason)); }

    if (last.status === "fulfilled") setLastSettlement(last.value.data || null);
    else setLastSettlement(null);

    if (adv.status === "fulfilled") setAdvances(adv.value.data || []);
    else { setAdvances([]); toast.error(formatApiError(adv.reason)); }
  ...
  const confirmSettle = async () => {
    ...
    if (previewMath && previewMath.advanceTotal > previewMath.net) {
      return toast.error("Selected advance payments exceed the net payable amount");
    }
    setConfirming(true);
    try {
      const r = await api.post("/procurement/settlements", {
        ...
        advance_payment_ids: selectedAdvanceIds,
      });
      setReceipt(r.data);
      setPreview(null);
  ...
  return (
    <div className="space-y-8" data-testid="procurement-settlement-page">
      ...
      <section className="bg-white border border-[#E7E5E4] p-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-dashed border-[#E7E5E4]">
          ...
          <button onClick={confirmSettle}
            disabled={!preview || preview.entry_count === 0 || confirming || deductionPercent === null || (previewMath && previewMath.advanceTotal > previewMath.net)}
      ...
      {preview && (
        <PreviewPanel
          preview={preview} math={previewMath} deductionPercent={deductionPercent}
          advances={advances} selectedAdvanceIds={selectedAdvanceIds}
          onToggleAdvance={(id) => setSelectedAdvanceIds((prev) =>
        ...
        <DialogContent className="bg-[#F9F8F6] border border-[#E7E5E4] rounded-sm max-w-md" data-testid="receipt-dialog">
          ...
          {receipt && (
            <dl className="mt-2 divide-y divide-[#E7E5E4] border-y border-[#E7E5E4]">
              ...
              <RRow label="Net payable" value={formatCurrency(receipt.net_paid)} testid="receipt-net" mono />
              {receipt.advance_total > 0 && (
                <>
                  <RRow label="Advance applied" value={`− ${formatCurrency(receipt.advance_total)}`} testid="receipt-advance" mono />
                  <RRow label="Paid now (posted)" value={formatCurrency(receipt.amount_paid_now)} testid="receipt-paid-now" mono emphasis />
                </>
              )}
            </dl>
          )}
...
function PreviewPanel({ preview, math, deductionPercent, advances, selectedAdvanceIds, onToggleAdvance }) {
  const { t } = useLang();
  const empty = preview.entry_count === 0;
  const advanceOver = math && math.advanceTotal > math.net;
  return (
    <section className="space-y-6" data-testid="preview-panel">
        ...
        {empty ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              {math.advanceTotal > 0 && (
                <Stat label="Advance applied"
                  value={`− ${formatCurrency(math.advanceTotal)}`} testid="preview-advance" mono tone="negative" />
              )}
              <div className="col-span-2 md:col-span-1 bg-[#FEF3C7] border-2 border-[#B45309] p-4">
                <div className="text-[10px] uppercase tracking-widest text-[#92400E]">
                  {math.advanceTotal > 0 ? "Pay now" : t("set.netPayable")}
                </div>
                <div className="mt-1 font-mono font-bold text-3xl text-[#B45309]" data-testid="preview-net">
                  {formatCurrency(math.advanceTotal > 0 ? math.paidNow : math.net)}
                </div>
              </div>
            </div>
            {advanceOver && (
              <div className="mt-4 flex items-start gap-3 bg-red-50 border border-red-200 p-4 text-sm text-red-900" data-testid="advance-over-error">
                <AlertTriangle strokeWidth={1.5} className="w-4 h-4 mt-0.5 flex-shrink-0" />
                <div>Selected advance payments exceed the net payable amount — unselect one to continue.</div>
              </div>
            )}
          </>
        )}
      ...
      {!empty && (
        <div className="bg-white border border-[#E7E5E4]" data-testid="advances-panel">
          <div className="px-5 py-3 border-b border-[#E7E5E4] bg-[#F5F4F0]">
            <h3 className="text-sm uppercase tracking-widest text-[#1C1917] font-bold flex items-center gap-2">
              <Wallet strokeWidth={1.5} className="w-4 h-4" />
              Advances — apply an earlier payment against this settlement
            </h3>
            <p className="text-xs text-stone-500 mt-1">
              Muddat is always calculated on the full gross above. Select any advance already paid to this client to subtract it from the amount you pay now.
            </p>
          </div>
          {advances.length === 0 ? (
            <div className="px-5 py-4 text-sm text-stone-500" data-testid="advances-empty">
              No unlinked payments available for this client.
            </div>
          ) : (
            <ul className="divide-y divide-[#E7E5E4]" data-testid="advances-list">
              {advances.map((a) => {
                const checked = selectedAdvanceIds.includes(a.id);
                return (
                  <li key={a.id}>
                    <label className="flex items-center gap-3 px-5 py-3 cursor-pointer hover:bg-[#FAFAF9]" data-testid={`advance-row-${a.id}`}>
                      <input type="checkbox" checked={checked} onChange={() => onToggleAdvance(a.id)}
                        data-testid={`advance-checkbox-${a.id}`}
                        className="h-4 w-4 accent-[#B45309]" />
                      <div className="flex-1 flex items-center justify-between">
                        <div>
                          <div className="text-sm text-[#1C1917]">{a.description || "Payment"}</div>
                          <div className="text-xs text-stone-500 font-mono">{formatDate(a.entry_date)}</div>
                        </div>
                        <div className="font-mono font-semibold text-[#1C1917]">{formatCurrency(a.amount)}</div>
                      </div>
                    </label>
                  </li>
                );
