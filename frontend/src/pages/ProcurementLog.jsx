...
import {
  ...
  Download, FileDown, Settings, Trash2, X, ListChecks, Pencil, ClipboardList,
} from "lucide-react";
 ...
 * - Batch flow: each entry the accountant fills gets reviewed, then added to
 *   a local "pending batch" (nothing hits the server yet). Pending entries
 *   persist to localStorage so a refresh doesn't lose them. When he's done,
 *   "Review & Save All" opens a dialog listing every pending entry — each
 *   editable/deletable — and only on final confirm does the whole batch get
 *   POSTed to the server together.
 ...
 */
const PENDING_BATCH_KEY = "pp_procurement_pending_batch";
export default function ProcurementLog() {
  ...
  // per-entry review dialog (before adding to the pending batch — no API call yet)
  const [reviewOpen, setReviewOpen] = useState(false);

  // pending batch: entries the accountant has built up locally, not yet saved
  ...
  const [editingLocalId, setEditingLocalId] = useState(null); // set while editing a pending entry from the batch dialog

  // final "review whole batch" dialog & its save-all action
  ...
  // Keep the pending batch persisted so a refresh/accidental close doesn't lose it.
  useEffect(() => {
    try {
      localStorage.setItem(PENDING_BATCH_KEY, JSON.stringify(pendingEntries));
    } catch {
      // best-effort; not fatal if storage is unavailable/full
    }
  }, [pendingEntries]);
  ...
  const resetForm = () => {
    ...
    setClientId("");
    // Product is intentionally kept selected so the accountant doesn't have
    // to re-pick it for consecutive entries of the same product.
    setWeight("");
    setRate("");
    setEditingLocalId(null);
  };

  // Step 1: form submit → validate → open per-entry review dialog. NO API call yet.
  const openReview = (e) => {
    e.preventDefault();
  ...
  // Step 2: user confirmed this single entry → add it to the local pending
  // batch (or update it in place if we're editing an existing pending entry).
  // Nothing is saved to the server here.
  const confirmAddToBatch = () => {
    const localId = editingLocalId || `local_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const entry = {
      localId,
    ...
    setPendingEntries((prev) => {
      if (editingLocalId) {
        return prev.map((it) => (it.localId === editingLocalId ? entry : it));
      }
      return [...prev, entry];
    });

    toast.success(editingLocalId ? "Entry updated in the pending batch" : "Added to pending batch");
    setReviewOpen(false);
    resetForm();
  };
  ...
  const removeFromBatch = (localId) => {
    setPendingEntries((prev) => prev.filter((it) => it.localId !== localId));
  };

  const editFromBatch = (entry) => {
  ...
  const pendingTotals = useMemo(() => {
    return pendingEntries.reduce(
      (acc, e) => ({
        weight: acc.weight + Number(e.weight || 0),
  ...
  // Final step: whole batch reviewed & confirmed → save it all together.
  const confirmSaveAllBatch = async () => {
    if (pendingEntries.length === 0) return;
    setSavingBatch(true);
    try {
      const res = await api.post("/procurement/entries/batch", {
      ...
      setBatchReviewOpen(false);
      await fetchTodaysEntries();
      toast.success(`Saved ${res.data.length} ${res.data.length === 1 ? "entry" : "entries"}`);
    } catch (err) {
      toast.error(formatApiError(err));
    } finally {
  ...
  return (
    <div className="space-y-8" data-testid="procurement-log-page">
      ...
      <form onSubmit={openReview} data-testid="procurement-form" className="bg-white border border-[#E7E5E4] p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-end justify-between gap-4 pt-3 border-t border-dashed border-[#E7E5E4]">
          ...
          <div className="flex items-center gap-2">
            {editingLocalId && (
              <button type="button" onClick={resetForm} data-testid="cancel-edit-pending-btn"
                className="border border-[#D6D3D1] text-[#292524] px-4 py-3 text-sm uppercase tracking-widest hover:bg-[#F0EFEA] transition-colors">
                Cancel Edit
              </button>
            )}
            <button type="submit" data-testid="procurement-submit-btn"
              ...
              {editingLocalId ? "Review & Update Entry" : t("log.reviewAndSave")}
            </button>
          </div>
        </div>
      </form>

      {/* Pending batch bar — entries added locally, not yet saved to the server */}
      {pendingEntries.length > 0 && (
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-[#FFFBEB] border border-[#F5D48A] px-5 py-4"
          data-testid="pending-batch-bar">
          <div className="flex items-center gap-2 text-sm text-[#92400E]">
            <ClipboardList strokeWidth={1.5} className="w-4 h-4 shrink-0" />
            <span data-testid="pending-batch-count">
              <strong>{pendingEntries.length}</strong> {pendingEntries.length === 1 ? "entry" : "entries"} pending ·{" "}
              <span className="font-mono">{pendingTotals.weight.toLocaleString("en-IN")} qtl</span> ·{" "}
              <span className="font-mono">{formatCurrency(pendingTotals.amount)}</span> — not saved yet
            </span>
          </div>
          <button onClick={() => setBatchReviewOpen(true)} data-testid="review-save-all-btn"
            className="inline-flex items-center justify-center gap-2 bg-[#292524] text-[#FAFAF9] px-5 py-2.5 text-xs uppercase tracking-widest hover:bg-[#1C1917] transition-colors">
      ...
      {/* Per-entry Review Dialog — adds to the local pending batch, does NOT hit the server */}
        ...
        <DialogContent className="bg-[#F9F8F6] border border-[#E7E5E4] rounded-sm max-w-md" data-testid="review-dialog">
          <DialogHeader>
            <DialogTitle className="text-2xl font-serif text-[#1C1917]">
              {editingLocalId ? "Review updated entry" : "Review before adding"}
            </DialogTitle>
            <DialogDescription className="text-sm text-stone-500">
              Nothing has been saved to the server yet. Verify the details, then add it to the
              pending batch — you can review and edit the whole batch before final save.
            </DialogDescription>
          </DialogHeader>
          <dl className="mt-2 divide-y divide-[#E7E5E4] border-y border-[#E7E5E4]">
            <DetailRow label="Date" value={formatDate(entryDate)} testid="review-date" />
            <DetailRow label="Client Name" value={formatClientName(selectedClient?.name ?? "—")} testid="review-client" />
            <DetailRow label="Product" value={selectedProduct?.name ?? "—"} testid="review-product" />
            <DetailRow label="Weight" value={`${Number(weight || 0).toLocaleString("en-IN")} Quintal`} testid="review-weight" mono />
            <DetailRow label="Rate" value={`${formatCurrency(Number(rate || 0))} / Quintal`} testid="review-rate" mono />
            <DetailRow label="Total Amount" value={formatCurrency(totalAmount)} testid="review-total" mono emphasis />
          </dl>
          <DialogFooter className="gap-2 pt-4">
            <button onClick={() => setReviewOpen(false)} data-testid="review-cancel-btn"
              className="border border-[#D6D3D1] text-[#292524] px-5 py-2 text-sm uppercase tracking-widest hover:bg-[#F0EFEA]">
              Cancel / Edit
            </button>
            <button onClick={confirmAddToBatch} data-testid="review-save-btn"
              className="bg-[#B45309] text-[#FAFAF9] px-5 py-2 text-sm uppercase tracking-widest hover:bg-[#92400E] inline-flex items-center gap-2">
              <Check strokeWidth={1.5} className="w-4 h-4" />
              {editingLocalId ? "Update in Batch" : "Add to Batch"}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Final Batch Review Dialog — lists every pending entry, editable/deletable, saves them all together */}
      <Dialog open={batchReviewOpen} onOpenChange={setBatchReviewOpen}>
        <DialogContent className="bg-[#F9F8F6] border border-[#E7E5E4] rounded-sm max-w-2xl" data-testid="batch-review-dialog">
          <DialogHeader>
            <DialogTitle className="text-2xl font-serif text-[#1C1917]">Review Pending Batch</DialogTitle>
            <DialogDescription className="text-sm text-stone-500">
              Nothing has been saved yet. Check each entry below — edit or remove any of them —
              then confirm to save the whole list together.
            </DialogDescription>
          </DialogHeader>

          {pendingEntries.length === 0 ? (
            <div className="py-8 text-center text-stone-500 italic">Batch is empty.</div>
          ) : (
            <div className="max-h-[50vh] overflow-y-auto border border-[#E7E5E4] bg-white">
              <table className="w-full text-sm">
                <thead className="bg-[#F5F4F0] text-[11px] uppercase tracking-widest text-stone-500 sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-[#E7E5E4]">
                  {pendingEntries.map((e, idx) => (
                    <tr key={e.localId} data-testid={`pending-row-${idx}`}>
                      <td className="px-4 py-2.5 font-mono text-[12px] text-stone-500">{formatDate(e.entry_date)}</td>
                      <td className="px-4 py-2.5">{formatClientName(e.client_name)}</td>
                      <td className="px-4 py-2.5">{e.product_name}</td>
                      <td className="px-4 py-2.5 text-right font-mono">{Number(e.weight).toLocaleString("en-IN")}</td>
                      <td className="px-4 py-2.5 text-right font-mono">{formatCurrency(e.rate)}</td>
                      <td className="px-4 py-2.5 text-right font-mono font-semibold">{formatCurrency(e.total_amount)}</td>
                      <td className="px-4 py-2.5">
                        <div className="flex items-center justify-end gap-3">
                          <button onClick={() => editFromBatch(e)} data-testid={`pending-edit-${idx}`}
                            className="text-stone-500 hover:text-[#292524]" title="Edit">
                            <Pencil strokeWidth={1.5} className="w-4 h-4" />
                          </button>
                          <button onClick={() => removeFromBatch(e.localId)} data-testid={`pending-delete-${idx}`}
                            className="text-stone-500 hover:text-red-700" title="Remove">
                ...
                <tfoot className="bg-[#FAFAF9] border-t-2 border-[#78716C] text-[#1C1917]">
                  <tr>
                    <td colSpan={3} className="px-4 py-2.5 text-[11px] uppercase tracking-widest text-stone-500">
                      Batch Total
                    </td>
                    <td className="px-4 py-2.5 text-right font-mono font-semibold">
                      {pendingTotals.weight.toLocaleString("en-IN")}
                    </td>
                    <td></td>
                    <td className="px-4 py-2.5 text-right font-mono font-semibold">
                      {formatCurrency(pendingTotals.amount)}
                    </td>
                    <td></td>
                  </tr>
                </tfoot>
          ...
          <DialogFooter className="gap-2 pt-4">
            <button onClick={() => setBatchReviewOpen(false)} disabled={savingBatch} data-testid="batch-review-close-btn"
            ...
            <button onClick={confirmSaveAllBatch} disabled={savingBatch || pendingEntries.length === 0}
              data-testid="batch-confirm-save-btn"
              ...
              <Check strokeWidth={1.5} className="w-4 h-4" />
              {savingBatch ? "Saving…" : `Confirm & Save All (${pendingEntries.length})`}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
