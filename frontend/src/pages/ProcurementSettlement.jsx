...
import {
  Wallet, Check, ChevronsUpDown, CalendarDays, Percent, Receipt, History, AlertTriangle,
} from "lucide-react";
...
/**
 * MODULE — Procurement Payment Settlement (watermark-based)
 * Advances client's "settled_until" watermark to cutoff_date each settlement.
 * Eligible entries: entry_date > previous_watermark AND entry_date <= cutoff_date.
 */
export default function ProcurementSettlement() {
  ...
  // deduction — "1" | "1.5" | "custom"
  const [dedKey, setDedKey] = useState("1");
  const [customPct, setCustomPct] = useState("");

  const [preview, setPreview] = useState(null);   // {entry_count, gross_amount, settled_from_date, cutoff_date}
  const [loadingPreview, setLoadingPreview] = useState(false);
  const [confirming, setConfirming] = useState(false);
  const [settlements, setSettlements] = useState([]);

  const [receipt, setReceipt] = useState(null);   // last-settled record for dialog
  ...
  // Load settlement history whenever client changes
  const loadHistory = async (cid) => {
    if (!cid) { setSettlements([]); return; }
    try {
      const r = await api.get(`/procurement/settlements?client_id=${cid}`);
  ...
  const runPreview = async () => {
    ...
    if (!cutoffDate) return toast.error("Choose a cutoff date");
    if (deductionPercent === null) return toast.error("Enter a valid deduction % (0–100)");
    setLoadingPreview(true);
    setPreview(null);
    try {
      const r = await api.get(`/procurement/clients/${clientId}/outstanding?cutoff_date=${cutoffDate}`);
      setPreview(r.data);
      if (r.data.entry_count === 0) {
        toast.info("No unsettled entries in this window.");
      }
    } catch (e) {
      toast.error(formatApiError(e));
    } finally {
  ...
  const confirmSettle = async () => {
    ...
    if (preview.entry_count === 0) return toast.error("Nothing to settle in this window");
    ...
    <div className="space-y-8" data-testid="procurement-settlement-page">
      ...
      <header className="flex items-baseline justify-between border-b border-[#E7E5E4] pb-4">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Pay a client for all unpaid procurement, up to a chosen date.</p>
        </div>
      </header>
      ...
      <section className="bg-white border border-[#E7E5E4] p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div>
            <label className="text-[11px] uppercase tracking-widest text-stone-500 flex items-center gap-1.5">
              <CalendarDays strokeWidth={1.5} className="w-3.5 h-3.5" /> Cutoff date
            </label>
            <input type="date" value={cutoffDate} onChange={(e) => setCutoffDate(e.target.value)}
              max={todayIso} data-testid="settlement-cutoff-input"
            ...
            <p className="mt-1 text-[10px] text-stone-400">Settles entries up to and including this date.</p>
          </div>
          ...
          <div>
            ...
            {dedKey === "custom" && (
              <div className="mt-3">
                <div className="flex items-baseline gap-2 border-b border-[#D6D3D1]">
                ...
                <p className="mt-1 text-[10px] text-stone-400">Between 0 and 100.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 pt-2 border-t border-dashed border-[#E7E5E4]">
          <button onClick={runPreview} disabled={loadingPreview || !clientId}
            ...
            <Receipt strokeWidth={1.5} className="w-4 h-4" />
            {loadingPreview ? "Calculating…" : "Preview Outstanding"}
          </button>
          ...
          {selectedClient && (
            <span className="text-xs text-stone-500 font-mono ml-auto" data-testid="context-summary">
              {formatClientName(selectedClient.name)} · cutoff {formatDate(cutoffDate)}
              {deductionPercent !== null && ` · ${deductionPercent}% deduction`}
            </span>
          )}
        </div>
      </section>
      ...
      {/* Preview panel */}
      {preview && previewMath && (
        <section className="bg-white border border-[#E7E5E4] p-6" data-testid="preview-panel">
          <div className="text-xs uppercase tracking-widest text-stone-500 mb-4">
            Preview · {formatClientName(preview.client_name)} · {preview.settled_from_date
              ? <>previously settled through <span className="font-mono">{formatDate(preview.settled_from_date)}</span></>
              : <>first settlement (nothing previously settled)</>}
          </div>
          ...
          {preview.entry_count === 0 ? (
            <div className="flex items-start gap-3 bg-orange-50 border border-orange-200 p-4 text-sm text-orange-900" data-testid="preview-empty">
              ...
              <div>
                No unsettled procurement entries between
                {" "}<strong>{preview.settled_from_date ? formatDate(preview.settled_from_date) : "beginning"}</strong> (exclusive)
                {" "}and <strong>{formatDate(preview.cutoff_date)}</strong> (inclusive).
                {" "}Nothing to settle.
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <Stat label="Gross amount" value={formatCurrency(previewMath.gross)} testid="preview-gross" mono />
              <Stat label={`Deduction (${deductionPercent}%)`} value={`− ${formatCurrency(previewMath.deduction)}`} testid="preview-deduction" mono tone="orange" />
              <Stat label="Net payable" value={formatCurrency(previewMath.net)} testid="preview-net" mono emphasis />
            </div>
          )}
        </section>
      )}
            ...
            <table className="w-full text-sm" data-testid="settlement-history-table">
              <thead className="bg-[#F5F4F0] text-[11px] uppercase tracking-widest text-stone-500">
                <tr>
                  ...
                  <th className="text-left px-5 py-3 font-medium">Window (from → to)</th>
                ...
                {settlements.length === 0 ? (
                  ...
                  <tr key={s.id} className="hover:bg-[#FAFAF9]" data-testid={`settlement-row-${s.id}`}>
                    ...
                    <td className="px-5 py-3 font-mono text-[12px]">
                      {s.settled_from_date ? formatDate(s.settled_from_date) : "—"}
                      {" → "}
                      {formatDate(s.settled_to_date)}
                    </td>
        ...
        <DialogContent className="bg-[#F9F8F6] border border-[#E7E5E4] rounded-sm max-w-md" data-testid="receipt-dialog">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-stone-500">
              Recorded in the ledger. Watermark advanced.
            </DialogDescription>
          </DialogHeader>
          {receipt && (
            <dl className="mt-2 divide-y divide-[#E7E5E4] border-y border-[#E7E5E4]">
              ...
              <RRow label="Window" value={`${receipt.settled_from_date ? formatDate(receipt.settled_from_date) : "beginning"} → ${formatDate(receipt.settled_to_date)}`} testid="receipt-window" mono />
              ...
              <RRow label="Net paid" value={formatCurrency(receipt.net_paid)} testid="receipt-net" mono emphasis />
            </dl>
          )}
