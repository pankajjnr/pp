...
export default function Layout() {
  ...
  // Used less often — tucked away under "More" instead of always taking up nav space
  const moreItems = [
  ...
  const isMoreActive = moreItems.some((n) => location.pathname === n.to || location.pathname.startsWith(n.to + "/"));
  ...
  return (
    <div className="min-h-screen bg-[#f9f8f6] paper-grain">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    ...
                    className={`px-3 py-2 text-sm flex items-center gap-2 transition-colors outline-none ${
                      isMoreActive ? "bg-[#292524] text-[#FAFAF9]" : "text-stone-700 hover:bg-[#F0EFEA]"
                    }`}
                  >
                ...
                <DropdownMenuContent align="start" className="min-w-[200px]">
                  {moreItems.map((n) => (
                    <DropdownMenuItem key={n.to} asChild data-testid={n.testid}>
                      <NavLink
                        to={n.to}
                        className={({ isActive }) =>
                          `flex items-center gap-2 cursor-pointer ${isActive ? "font-medium" : ""}`
                        }
                      >
                        <n.icon strokeWidth={1.5} className="w-4 h-4" />
                        {n.label}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className={`px-3 py-2 text-xs flex items-center gap-1 whitespace-nowrap outline-none ${
                    isMoreActive ? "bg-[#292524] text-white" : "text-stone-700"
                  }`}
                >
              ...
              <DropdownMenuContent align="start" className="min-w-[200px]">
                {moreItems.map((n) => (
                  <DropdownMenuItem key={n.to} asChild data-testid={`${n.testid}-mobile`}>
                    <NavLink
                      to={n.to}
                      className={({ isActive }) =>
                        `flex items-center gap-2 cursor-pointer ${isActive ? "font-medium" : ""}`
      ...
      <footer className="border-t border-[#E7E5E4] mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 text-xs text-stone-500 flex justify-end">
