...
class AvailableAdvanceOut(BaseModel):
    id: str
    amount: float
    description: Optional[str] = None
    entry_date: str
...
@api_router.get(
    "/procurement/clients/{client_id}/available-advances",
    response_model=List[AvailableAdvanceOut],
)
async def available_advances(client_id: str, _user: dict = Depends(get_current_user)):
    """Payments already made to this client that are not yet linked to any
    settlement — candidates the user can manually apply against a new
    settlement's payable amount (e.g. an advance given ahead of time)."""
    rows = await db.payments.find({
        "client_id": client_id,
        "direction": "out",
        "settlement_id": {"$exists": False},
    }, {"_id": 0}).sort("entry_date", 1).to_list(5000)
    return [
        AvailableAdvanceOut(
            id=r["id"],
...
async def create_settlement(payload: SettlementCreate, _user: dict = Depends(get_current_user)):
    ...
    # Advance payments the user manually chose to apply against this settlement.
    # Muddat (the deduction) is always computed on the full gross above — advances
    # never reduce the deduction base, only the cash actually paid out now.
    advance_docs = []
    if payload.advance_payment_ids:
        advance_docs = await db.payments.find({
            "id": {"$in": payload.advance_payment_ids},
        }, {"_id": 0}).to_list(1000)
        found_ids = {a["id"] for a in advance_docs}
        missing = set(payload.advance_payment_ids) - found_ids
        if missing:
            raise HTTPException(status_code=400, detail=f"Advance payment(s) not found: {', '.join(missing)}")
        for a in advance_docs:
            if a.get("client_id") != payload.client_id or a.get("direction") != "out":
                raise HTTPException(status_code=400, detail="Selected advance payment does not belong to this client")
            if a.get("settlement_id"):
                raise HTTPException(status_code=400, detail="Selected advance payment is already applied to another settlement")

    advance_total = round(sum(float(a.get("amount", 0)) for a in advance_docs), 2)
    if advance_total > net:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Selected advance payments ({advance_total}) exceed the net payable "
                f"amount ({net}) for this settlement."
            ),
        )
    ...
    if amount_paid_now > 0:
        advance_note = f" — advance applied: {advance_total}" if advance_total > 0 else ""
        payment_doc = {
            "id": payment_id,
            "client_id": payload.client_id,
            "client_name": c["name"],
            ...
            "description": (
                ...
                f"to {payload.to_date} (deduction {ded_pct}%){advance_note}"
            ),
    ...
    settlement_doc = {
        ...
        "net_paid": net,
        "advance_applied": advance_total,
        "amount_paid_now": amount_paid_now,
        "advance_payment_ids": [a["id"] for a in advance_docs],
    ...
    advances_linked = False
    advance_ids = [a["id"] for a in advance_docs]
    try:
        ...
        if advance_ids:
            adv_upd = await db.payments.update_many(
                {"id": {"$in": advance_ids}, "settlement_id": {"$exists": False}},
                {"$set": {"settlement_id": settlement_id}},
            )
            if adv_upd.modified_count != len(advance_ids):
                raise RuntimeError("Advance payment already applied elsewhere — concurrent settlement suspected")
            advances_linked = True
        ...
        if advances_linked:
            await db.payments.update_many(
                {"id": {"$in": advance_ids}, "settlement_id": settlement_id},
                {"$unset": {"settlement_id": ""}},
            )
        if flagged:
...
async def list_settlements(
    ...
    for r in rows:
        out.append(SettlementOut(
            ...
            net_paid=r["net_paid"],
            advance_applied=r.get("advance_applied", 0),
